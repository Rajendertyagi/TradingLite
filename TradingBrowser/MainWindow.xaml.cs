...
namespace TradingBrowser
{
    public partial class MainWindow : Window
    {
        ...
        private readonly AdBlockService _adBlockService = new AdBlockService();
        ...
        private async void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            _environment = await CoreWebView2Environment.CreateAsync(null, folder, new CoreWebView2EnvironmentOptions { AdditionalBrowserArguments = "--disable-background-timer-thttping --disable-renderer-backgrounding" });
            ...
            {
                if (e.PropertyName == nameof(ViewModel.IsSiteWhslitelisted) && ViewModel.SelectedTab != null)
        ...
        { 
            // Intentionally empty so we don't save state 
        } 
        ...
        { 
            ...
            { 
                ...
                ViewModel.IsSiteWhitelist = _adBlocker.IsDomainWhitelisted(ViewModel.SelectedTab.Url); 
            } 
        }
        ...
        private async System.Threading.Tasks.Task ActivateTab(TabViewModel tab)
        {
            if (!_webViewPool.ContainsKey(tab.Id))
            {
                ...
                _adBlocker.AttachToWebView(webView2.CoreWebView2);
                ...
                { 
                    ...
                    { 
                        try { tab.Title = webView.CoreWebView2.DocumentTitle; } 
                        catch { } 
                    }); 
                };
                        ...
                        if (download != null && !string.IsNullOrEmpty(download.ResultFilePath))
                        {
                            ...
                            { 
                                ...
                                { 
                                    Application.Current?.Dispatcher?.Invoke(() => 
                                    { 
                                        item.BytesReceived = (long)download.BytesReceived; 
                                        item.TotalBytes = (long)(download.TotalBytesToReceive ?? 0); 
                                    }); 
                                } 
                                catch { } 
                            }; 
                            ...
                            { 
                                ...
                                { 
                                    if (download.State == CoreWebView2DownloadState.Completed || download.State == CoreWebView2DownloadState.Interrupted) 
                                        Application.Current?.Dispatcher?.Invoke(() => item.IsComplete = true); 
                                } 
                                catch { } 
                            }; 
        ...
        private void EditHomepage()
        {
            try
            {
                ...
                { 
                    var def = GetDefaultLinks(); 
                    File.WriteAllText(_homePath, JsonSerializer.Serialize(def, new JsonSerializerOptions { WriteIndented = true })); 
                }
                System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo("notepad.exe", _homePath) { UseShellExecute = true };
            }
            catch (Exception ex) 
            { 
                MessageBox.Show($"Failed to open editor: {ex.Message}"); 
            }
        }

        private class HomePageLink 
        { 
            public string Title { get; set; } = ""; 
            public string Url { get; set; } = ""; 
            public string Color { get; set; } = "#0078d9d4"; 
            public string IconEmoji { get; set; } = "📈"; 
        }

        private List<HomePageLink> GetDefaultLinks() 
        { 
            return new List<HomePageLink> 
            { 
                new() { Title = "Zerodha", Url = "https://kite.zerodha.com", Color = "#387ed1", IconEmoji = "📈" }, 
                new() { Title = "Upstox", Url = "https://pro.upstox.com", Color = "#5e72e4", IconEmoji = "📈" }, 
                new() { Title = "Groww", Url = "https://app.groww.in", Color = "#00d09c", IconEmoji = "📈" }, 
                new() { Title = "Angel One", Url = "https://trade.angelone.in", Color = "#e8375d", IconEmoji = "📈" }, 
                new() { Title = "ICICI Direct", Url = "https://www.icicidirect.com", Color = "#f37021", IconEmoji = "📈" }
            }; 
        }
        ...
        private void LoadHomePage(Microsoft.WebView2.Wpf.WebView2 webView)
        {
            ...
            if (File.Exists(_homePath))
            {
                try 
                { 
                    links = JsonSerializer.Deserialize<List<HomePageLink>>(File.ReadAllText(_homePath)) ?? new List<HomePageLink>(); 
                } 
                catch 
                { 
            ...
            { 
                links = GetDefaultLinks(); 
                File.WriteAllText(_homePath, JsonSerializer.Serialize(links, new JsonSerializerOptions { WriteIndented = true })); 
            }

            string linksJson = JsonSerializer.Serialize(links);
            
            string html = @"<html><head><style>body{background-color:#1e1e1e;color:white;font-family:'Inter',s-serif;display:flex;flex-direction:column;align-items:center;margin-top:10%}.brokers{display:flex;gap:20px;margin-top:30px;flex-wrap:wrap;justify-content:center}.broker-card{background:#2d2d2d;padding:20px;border-radius:10px;text-align:center;cursor:pointer;transition:0.2s;text-decoration:none;color:white;width:120px}.broker-card:hover{background:#3d3d3d;transform:scale(1.05)}.edit-btn{position:absolute;top:20px;right:20px;background:#35363a;border:1px solid #444;color:white;padding:8px 12px;border-radius:6px;cursor:pointer;font-size:13px}.edit-btn:hover{background:#4a4b4f}input[type='text']{margin-top:40px;padding:12px 20px;width:500px;border-radius:25px;border:1px solid #444;background:#333;color:white;font-size:16px;outline:none}input[type='text']:focus{border-color:#0078d4}</style></head><body><button class='edit-btn' onclick=""window.chrome.webview.postMessage('edit_homepage')"">✏️ Edit Homepage</button><h2>My Quick Access</h2><div class='brokerList' id='brokerList'></div><input type='text' id='searchbox' placeholder='Search Google or type a URL... (e.g. tv RELIANCE)' autofocus><script>const links=" + linksJson + @";const list=document.getElementById('brokerList');links.forEach(link=>{const a=document.createElement('a');a.className='broker-card';a.href=link.url;a.innerHTML=`<div style='text-align:center;'><div style='font-size:30px;'>${link.IconEmoji}</div><h3 style='color:${link.color};margin-top:4px;font-size:12px;'>${link.title}</h3></div>`;list.appendChild(a)});document.getElementById('searchbox').addEventListener('keydown',function(e){if(e.key==='Enter'){let val=this.value;if(!val.includes('.')||val.includes(' '))val='https://www.google.com/search?q='+encodeURIComponent(val);else if(!val.startsWith('http'))val='https://'+val;window.chrome.webview.postMessage(val)}});document.querySelectorAll('.broker-card').forEach(a=>{a.addEventListener('click',function(e){e.preventDefault();window.chrome.webview.postMessage(this.href)})});</script></body></html>";
            webView.CoreWebView2.NavigateToString(html);
        }

        private void LoadSettingsPage(Microsoft.WebView2.Wpf.WebView2 webView) 
        { 
            webView.CoreWebView2.NavigateToString(@"<html><head><style>body{background-color:#202124;color:#e8eaed;font-family:'Inter',sans-serif;padding:40px}h2{margin-top:0}button{padding:12px 20px;background:#0078d4;color:white;border:none;border-radius:4px;cursor:pointer;font-size:14px;margin-top:20px}button:hover{background:#1a86d9}.shortcut{background:#35363a;padding:10px;margin-top:10px;border-radius:4px}</style></head><body><h2>Settings</h2><p><strong>Ad-Blocker:</strong> Active. Use shield icon in address bar.</p><p><strong>Quick Menu (TradingView):</strong></p><div class='shortcut'>Ctrl + Alt + 1 &rarr; 1 Min</div><div class='shortcut'>Ctrl + Alt + 5 &rarr; 1 Hour</div><button onclick=""window.chrome.webview.postMessage('clearcache')"">Clear All Cache</button></body></html>"); 
        }

        private void MuteTab(Microsoft.WebView2.Wpf.WebView2 webView, bool mute) 
        { 
            try 
            { 
                if(webView.CoreWebView2!=null) 
                    webView.CoreWebView2.CallDevToolsProtocolMethodAsync("Audio.setMuted", mute ? "{\"muted\":true}" : "{\"muted\":false}").ConfigureAwait(false); 
            } 
            catch { } 
        }
        ...
        { 
            ...
            { 
                ...
                if (<<<<<< Wait, missing an operator here! 
        ...
        { 
            if (e.Data.GetData(typeof(TabViewModel)) is TabViewModel) 
        ...
        { 
            try 
            { 
                if (e.Data.GetData(typeof(TabViewModel)) is TabViewModel droppedTab && sender is FrameworkElement target && target.DataContext is TabViewModel targetTab && droppedTab.Id != targetTab.Id) 
                { 
                    int oldIndex = ViewModel.Tabs.IndexOf(droppedTab); 
                    int newIndex = ViewModel.Tab`
                } 
            } 
            catch { } 
        }
        ...
        private void Window_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            ...
            { 
                ViewModel.UndoCloseTabCommand.Execute(null); 
                e.Handled = true; 
            }
            ...
            if (e.Key == Key.F11) 
            { 
                ViewModel.IsChartMode = !ViewModel.IsChartMode; 
                e.Handled = true; 
            }
        }
        ...
        { 
            SendMessage(System.Windows.Interop.WindowInteropHelper.Handle, WM_SYSCOMMAND, SC_MINIMIZE, 0); 
        }
        
        private void Maximize_Click(object sender, RoutedEventArgs e) 
        { 
            SendMessage(System.Windows.Interop.WindowInteropHelper.Handle, WM_SYSCOMMAND, SC_MAXIMIZE, 0); 
        }
